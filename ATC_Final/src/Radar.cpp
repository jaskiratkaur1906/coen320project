...
void* Radar::primaryRadar_client() {
    ...
    //timer initialized period of 1s
    cTimer timer(1, 0);

    //connection with computer system
    ...
    while (true) {
        ...
        for (auto &sCoid : IDVector) {
            ...
            if (MsgSend(sCoid, &msg, sizeof(msg), &request_msg, sizeof(request_msg)) == -1) {
                perror("Error while requesting aircraft info");
            ...
            if (MsgSend(csid, &cs_msg, sizeof(cs_msg), nullptr, 0) == -1) {
                perror("Error while sending to computer system");

                printf("Aircraft ID: %d, Position: (%d, %d, %d)\n", airData.id, airData.x, airData.y, airData.z);
    ...
    while (true) {
        ...
        for (const auto& aircraft : radarAircraft) {
            ...
            if (MsgSend(rcvid, &msg, sizeof(msg), &request_msg, sizeof(request_msg)) == -1) {
                perror("Error requesting data from aircraft");
                name_close(rcvid);
                continue;
            }
            ...
            if (MsgSend(csid, &cs_msg, sizeof(cs_msg), nullptr, 0) == -1) {
                perror("Error sending data to computer system");
                break;
            }
